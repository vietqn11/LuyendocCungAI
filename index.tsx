...
async function generateSpeech(text) {
    try {
        ...
        throw new Error('Không thể tạo giọng đọc mẫu của AI. Đang thử giọng đọc dự phòng.');
    }
}
...
function useAudioPlayer() {
  ...
  return { isPlaying, playAudio, stopAudio, playTextWithWebSpeech };
}
...
function useAudioRecorder() {
  ...
  const stopRecording = useCallback(() => {
    // FIX: Add explicit type to Promise and add a type guard for reader.result.
        ...
        mediaRecorderRef.current.onstop = () => {
          ...
          reader.onloadend = () => {
            if (typeof reader.result === 'string') {
              ...
              reject(new Error('Failed to read audio file as base64 string.'));
            }
          };
...
function ReadingView({ passage, onBack, onFinishRecording }) {
    ...
    try {
        ...
        try {
            await playTextWithWebSpeech(passage.content);
             ...
             setTtsError('Rất tiếc, cả hai hệ thống đọc mẫu đều đang gặp lỗi.');
        }
    } finally {
  ...
  const isBusy = isGeneratingSpeech;
        ...
        <div className="flex flex-col items-center">
          {!isRecording ? (
             <div className="flex items-center justify-center space-x-6">
              ...
              >
                {isGeneratingSpeech ? <i className="fas fa-spinner fa-spin"></i> : (isSamplePlaying ? <><i className="fas fa-stop mr-2"></i> Dừng nghe</> : <><i className="fas fa-volume-up mr-2"></i> Nghe mẫu</>)}
              </button>
              <button
                onClick={handleStart}
                disabled={isBusy || isSamplePlaying}
...
function EvaluationView({ result, studentInfo, passage, onChooseAnotherPassage, onReadAgain, sheetSaveStatus }) {
    ...
    const handlePlayWord = async (word) => {
        ...
        try {
            ...
            console.warn(`Gemini TTS failed for word "${word}", falling back to Web Speech API.`, error);
            try {
